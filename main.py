...
AUTO_EMOJI = {
    1397602383764262982: "<a:Legit:1399098372224323604>",  # 1. channel fix emoji
    1397602385244717187: "<a:whiteheart:1397855818543665303>"  # 2. channelhez még nincs fix emoji, random lesz
}
...
async def on_ready():
    ...
    async def on_message(message):
        ...
        if message.channel.id in CHANNEL_IDS:
            emoji = AUTO_EMOJI.get(message.channel.id)
            if emoji:
                try:
                    await message.add_reaction(emoji)
                except Exception as e:
